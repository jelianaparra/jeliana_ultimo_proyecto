...
import dbComponent.PoolMaganer;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			while(rs.next() && i < 10) {
				...
				jsonVideo.put("nombre", rs.getString("nombre"));
				jsonVideo.put("type", rs.getString("type"));
				jsonVideo.put("usuario", find.returnConstraintbyID(rs.getString("id_usuario")));
				ResultSet rsthumb = conn.exeQueryRS("select.thumbnails.where.id_usuario.nombre", new Object[] {rs.getInt("id_users"),
																											rs.getString("name")});
				while(rsthumb.next()) {
					JSONObject jsonThumbnail = new JSONObject();
					jsonThumbnail.put("nombre", rsthumb.getString("nombre"));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( sessions != null ) {
			...
			if(id != null)
			{
				...
				DBComponent conn = Pool.getDBInstance();
				try {
					ResultSet rs = conn.exeQueryRS("select.videos.where.id_usuario", new Object[] {Integer.parseInt(id)});
					int i = 0;
					while(rs.next() && i < 10) {
						JSONObject jsonVideo = new JSONObject();
						jsonVideo.put("nombre", rs.getString("nombre"));
						jsonVideo.put("type", rs.getString("type"));
						jsonVideo.put("username", find.returnConstraintbyID(rs.getString("id_usuario")));
						ResultSet rsthumb = conn.exeQueryRS("select.thumbnails.where.id_usuario.nombre", new Object[] {rs.getInt("id_users"),
																													rs.getString("name")});
						while(rsthumb.next()) {
							JSONObject jsonThumbnail = new JSONObject();
