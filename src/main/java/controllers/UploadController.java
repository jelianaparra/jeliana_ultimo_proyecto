...
public class UploadController {
	...
	public UploadController(String id, JSONObject json) throws SQLException {
		this.id = id;
		this.usuario = find.returnConstraintbyID(id);
	...
	private void uploadDB() throws SQLException {
		...
		String src = usuario + "/" + nombre + "." + type;
		DBComponent conn = Pool.getDBInstance();
		if(isVideo) {
			conn.exeSimple(new Query("insert.videos", new Object[] {Integer.parseInt(id), nombre, type, src, date}));
		}
		if(isThumbnail) {
			conn.exeSimple(new Query("insert.thumbnails", new Object[] {Integer.parseInt(id), nombre, type, src, date}));
		}
